...
#        img_ids = list(filter(lambda x: self._contain_large_enough_annotation(x), img_ids))
        img_ids = list(filter(lambda x: self._contain_large_annotation_only(x), img_ids))
    ...
    def _contain_large_annotation_only(self, img_id, min_w=10, min_h=10):
        ...
        for ann in anns:
            x, y, w, h = [int(j) for j in ann['bbox']]
            if ann['category_id'] in self.cat_ids and (w <= min_w or h <= min_h):
                return False
...
#                print('h<=10が検出されました {} {} このアノテーションは無視します'.format(file_name, h))
        ...
        if len(gt_boxes) == 0:
            print('小さすぎるアノテーションを削除した結果、この画像にはground_truthが一つも含まれませんでした。これで学習にエラーが出る場合、事前に小さなアノテーションしか含まれない画像はself.imgsから削除することを検討してください')
            
        # gt_masksはlistのままにしておいて、Transformでcv::resizeしたあとにnumpy arrayにするという泥臭
        return img, np.array(gt_boxes), np.array(gt_labels, dtype=np.int32), gt_masks 
