...
class FPNKeypointMaskRCNNTrainChain(FasterRCNNTrainChain):
    def __init__(self,
                 faster_rcnn,
                 rpn_sigma=3.,
                 roi_sigma=1.,
    ...
    def __call__(self, imgs, bboxes, keypoints, scale):
        if isinstance(bboxes, chainer.Variable):
            bboxes = bboxes.data
        if isinstance(keypoints, chainer.Variable):
        ...
        # gtはperson以外ない personはcat id = 0だけど、このあと+1される
